...
import { Search, UserPlus, Filter, Activity, ShieldCheck, Zap, Loader2, RefreshCcw, DatabaseZap } from 'lucide-react';
  ...
  const loadData = useCallback(async (silent = false) => {
    ...
    try {
      const [patientsRes, statusRes] = await Promise.all([
        fetch(`/api/patients${search ? `?q=${encodeURIComponent(search)}` : ''}`, { credentials: 'omit' }).then(r => r.json()),
      ...
      toast.error("Failed to sync with SQL production engine.");
    } finally {
      setLoading(false);
    ...
    try {
      ...
      if (data.success) {
        toast.success("SQL Registry Re-initialized", {
          description: `All ${data.count} records have been migrated to D1 persistence.`
        });
        await loadData();
      }
    } finally {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-10">
        <div className="space-y-8">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent className="p-4 w-72 bg-white dark:bg-card border-2 shadow-xl">
                      <div className="text-[10px] space-y-2 font-mono uppercase font-black">
                        ...
                        <p className="text-[8px] text-muted-foreground leading-tight pt-1 normal-case font-medium">Persistence Layer: Cloudflare D1 SQL Lite.</p>
                      </div>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <Card className="shadow-sm border-none bg-emerald-50/50 dark:bg-emerald-900/10">
          ...
          <div className="bg-card border rounded-[2rem] overflow-hidden shadow-soft">
            <div className="p-6 border-b flex items-center gap-6 bg-muted/20">
              <div className="relative flex-1">
                ...
                <Input
                  placeholder="Query SQL Registry by patient name or MRN..."
                  className="pl-11 h-12 rounded-2xl border-none shadow-inner bg-background focus-visible:ring-emerald-500"
                  value={search}
              ...
              <AnimatePresence mode="wait">
                 {isRefreshing && (
                   <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex items-center gap-2 text-xs font-bold text-muted-foreground pr-2">
                      <RefreshCcw className="h-3 w-3 animate-spin text-emerald-600" />
                      <span className="uppercase tracking-tighter">Querying SQL</span>
                   </motion.div>
                 )}
              </AnimatePresence>
            </div>
            <Table>
              ...
              <TableBody>
                {loading ? (
                  ...
                  <TableRow><TableCell colSpan={5} className="text-center py-40 text-muted-foreground font-bold">No SQL records found matching current query.</TableCell></TableRow>
                ) : (
                  patients.map((p) => (
                    <TableRow key={p.id} className="group hover:bg-emerald-50/50 dark:hover:bg-emerald-900/10 transition-colors">
                      ...
                      <TableCell>
                        <Badge variant="secondary" className="bg-emerald-100/50 text-emerald-800 dark:bg-emerald-900/40 dark:text-emerald-400 px-3 py-1 font-bold">
                          {p.diagnoses[0]?.description || 'SQL-RECORD'}
                        </Badge>
                      </TableCell>
                      <TableCell className="text-right px-8 py-5">
                        <Link to={`/provider/patient/${p.id}`}>
                          <Button variant="ghost" size="sm" className="rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-700 hover:text-white">Open Chart</Button>
                        </Link>
                      </TableCell>
                    </TableRow>
                  ))
            ...
            <div className="p-4 border-t bg-muted/10 text-center">
               <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">Data Engine: Cloudflare D1 SQL Lite</p>
            </div>
          </div>
        </div>
      </div>
