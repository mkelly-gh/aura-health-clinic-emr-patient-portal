...
export function Portal() {
  ...
  const handleAccessRecords = () => {
    toast.success("Synchronizing Records", {
      description: "Fetching most recent clinical telemetry from SQL node.",
      duration: 3000
    });
  ...
  if (!patient) return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center bg-slate-50">
      ...
      <Button onClick={() => window.location.href = '/'} className="mt-6 bg-slate-900 rounded-sm font-bold text-xs uppercase h-9 px-6">Return Home</Button>
    </div>
  );
  return (
      ...
      <header className="bg-white border-b px-6 py-3 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="h-7 w-7 rounded-sm bg-teal-700 flex items-center justify-center text-white shadow-clinical">
          ...
          <div className="flex items-center gap-4">
             ...
             <div className="h-8 w-8 rounded-sm bg-slate-100 border border-slate-200 flex items-center justify-center text-[10px] font-black text-slate-600">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <div className="lg:col-span-2 space-y-4">
            <div className="bg-slate-900 rounded-sm p-8 sm:p-12 text-white relative overflow-hidden border border-slate-800 shadow-clinical-bold">
              <div className="relative z-10">
                <Badge className="bg-teal-600/20 text-teal-400 border-teal-500/30 mb-6 px-2 py-0.5 rounded-sm font-black text-[9px] uppercase tracking-widest">
                ...
                <Button onClick={handleAccessRecords} className="bg-teal-700 hover:bg-teal-600 rounded-sm px-8 h-10 font-black text-[10px] uppercase tracking-widest transition-all">
                ...
                <CardContent className="p-4 space-y-4">
                  ...
                  <div className="flex gap-8">
                    ...
                    <div>
                      ...
                      <div className="font-mono text-[10px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded-sm mt-1 inline-block border border-slate-200">
                        {patient.mrn}
                      </div>
                    </div>
                  </div>
                ...
                <CardContent className="p-4 space-y-3">
                  {patient.medications.length > 0 ? patient.medications.slice(0, 3).map((m, i) => (
                    <div key={i} className="flex justify-between items-center text-[11px] pb-2 border-b border-slate-100 last:border-0 last:pb-0">
                      ...
                      <Badge className="bg-teal-50 text-teal-700 text-[8px] font-black h-4 px-1 rounded-sm border-none uppercase">Active</Badge>
                    </div>
                  )) : (
          ...
          <div className="space-y-4">
            <Card className="rounded-sm border-slate-200 bg-white p-6 shadow-clinical">
              <div className="flex items-center gap-3 mb-6">
                <div className="h-9 w-9 rounded-sm bg-slate-900 flex items-center justify-center text-teal-500 border border-slate-800 shadow-clinical">
              ...
              <p className="text-[11px] text-slate-600 leading-relaxed italic mb-8 font-medium bg-slate-50 p-3 border border-slate-100 rounded-sm">
              ...
              <Button onClick={() => setChatOpen(true)} className="w-full bg-slate-900 hover:bg-slate-800 rounded-sm h-10 font-black text-[10px] uppercase tracking-[0.2em]">Launch AI Assistant</Button>
            </Card>
            <Card className="rounded-sm border-slate-200 bg-slate-900 text-white p-6 shadow-clinical">
