...
export class ChatAgent extends Agent<Env, ChatState> {
  ...
  async onRequest(request: Request): Promise<Response> {
    try {
      ...
      if (method === 'POST' && url.pathname === '/init-context') {
        return this.handleInitContext(await request.json());
      }
  ...
  private async handleInitContext(body: { patientId: string }): Promise<Response> {
    ...
    const patient = await controller.getPatient(body.patientId);
    if (!patient) return Response.json({ success: false, error: "Patient not found" }, { status: 404 });
    const context = {
      summary: `${patient.firstName} ${patient.lastName}, ${patient.gender}, born ${patient.dob}. History: ${patient.history}`,
      activeMedications: patient.medications.filter(m => m.status === 'Active').map(m => `${m.name} ${m.dosage}`),
      recentDiagnoses: patient.diagnoses.map(d => `${d.code}: ${d.description} (${d.date})`)
    };
