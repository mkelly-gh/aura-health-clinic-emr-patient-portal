...
export class ChatHandler {
  ...
  private async handleNonStreamResponse(
    ...
    if (!responseMessage) return { content: 'I apologize, but I encountered an issue processing your request.' };
  ...
  private async generateToolResponse(
    ...
    const followUpCompletion = await this.client.chat.completions.create({
      model: this.model,
      messages: [
        { role: 'system', content: systemPromptOverride || 'You are a helpful AI assistant.' },
  ...
  private buildConversationMessages(userMessage: string, history: Message[], systemPromptOverride?: string) {
    const systemContent = systemPromptOverride || 'You are Dr. Aura, a medical assistant for Aura Health Clinic. Provide clear, professional guidance based on the patients clinical context. Always emphasize that you are an AI assistant.';
    return [
